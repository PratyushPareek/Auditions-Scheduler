...
for freshie in inSheet.iter_rows(min_row=minRow,max_row=maxRow,min_col=1,max_col=9):
    ...
    for i in range(0,6):
        if(status[i]):
            ...
            minStud += 1
            #Society Entry
            socSheets[i][t].cell(row=minSocs[i][t], column=1).value = rollNo
            socSheets[i][t].cell(row=minSocs[i][t], column=2).value = name
            socSheets[i][t].cell(row=minSocs[i][t], column=3).value = contact
            minSocs[i][t] += 1
            print("Done\n")
